...
class MaskGenerator():
    ...
    def __init__(self, height, width, channels=3, rand_seed=45, maskspath=None, randomlines=False):
        ...
        self.filepath = maskspath
        self.randomlines = randomlines

        # Seed for reproducibility
        if rand_seed:
            seed(rand_seed)

        with open(maskspath) as f:
            masks = f.readlines()
        msk = [eval(item.strip()) for item in masks]
        self.masks = []
        for m in msk:
            self.masks+=m
        self.offset = 10
    ...
    def _generate_mask(self):
        ...
        for _ in range(randint(1, 7)):
            box = self.masks[randint(1,len(self.masks)-1)]
            self.offset = randrange(1,15)
            ymin = box[0]
            ymax = box[2]
            xmin = box[1]
            xmax = box[3]
            img[ymin+self.offset:ymax-self.offset, xmin+self.offset:xmax-self.offset] = np.ones(3)
        return 1-img
